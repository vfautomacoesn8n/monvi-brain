...
*Atualizado em 2026-08-11 (Task 061), após verificação direta do código, dos testes e das decisões formais — não apenas da documentação anterior desta seção.*
...
- Fase 5 (operação de clientes e projetos): iniciada pela Task 053 (API CRUD real para `client` e `project`), estendida pela Task 054 (API real para `contact`, vinculado a `client`, e `project_membership`, vinculando pessoa a projeto com papel — encerramento de participação preserva histórico via `leftAt`, sem exclusão), pela Task 055 (entidade `task` nova no schema Drizzle — primeira entidade de schema criada nesta fase, não reaproveitada da Fase 2 — vinculada a `project`, com responsável opcional vinculado a `person`, e API CRUD correspondente), pela Task 056 (entidade `deliverable` nova, vinculada a `project` e opcionalmente a `task`, com ciclo de vida próprio — `draft`/`in_review`/`approved`/`delivered`/`rejected` — e API CRUD correspondente), pela Task 057 (entidade `approval` nova, vinculada a `deliverable` e a `person` (aprovador), registrando a decisão — `pending`/`approved`/`rejected` — com `decided_at` preenchido automaticamente ao decidir, e API CRUD correspondente), pela Task 058 (entidade `dependency` nova, auto-referenciada em `task` — uma tarefa depende de outra, com índice único evitando duplicatas e validação impedindo que uma tarefa dependa de si mesma — e API CRUD correspondente), pela Task 059 (entidade `risk` nova, vinculada a `project`, com severidade — `low`/`medium`/`high`/`critical` — status — `open`/`mitigated`/`closed` — e responsável opcional vinculado a `person`, e API CRUD correspondente), pela Task 060 (entidade `comment` nova, vinculada a `task` e a `person` (autor, sempre o usuário autenticado, nunca informado no corpo da requisição) — deliberadamente escopada a `task` apenas, sem associação polimórfica a outras entidades, para preservar integridade referencial real via FK — e API CRUD correspondente) e pela Task 061 (histórico de mudanças — sem tabela nova: rota `GET /history?entityType=...&entityId=...` genérica, lendo a tabela `audit_event` já existente desde a Fase 4, cobrindo as dez entidades da Fase 5). Todas sob autenticação e RBAC obrigatórios e sob a mesma suposição explícita de single-tenant, aguardando decisão formal sobre o modelo de multi-organização (seção 17) para serem revisadas quando essa decisão for tomada. O único entregável restante da fase são os dashboards operacionais mínimos. As migrações das tabelas `task` (`drizzle/0001_deep_scarlet_spider.sql`), `deliverable` (`drizzle/0002_nifty_electro.sql`), `approval` (`drizzle/0003_daffy_odin.sql`), `dependency` (`drizzle/0004_icy_pestilence.sql`), `risk` (`drizzle/0005_curious_synch.sql`) e `comment` (`drizzle/0006_equal_gladiator.sql`) foram geradas, passo que não depende de Docker, mas ainda não foram aplicadas contra um banco real. Validação da API contra um banco real segue o mesmo padrão da Fase 3: testes de integração criados (`apps/core-brain/tests/client-project.integration.test.ts`, `apps/core-brain/tests/contact-membership.integration.test.ts`, `apps/core-brain/tests/task.integration.test.ts`, `apps/core-brain/tests/deliverable.integration.test.ts`, `apps/core-brain/tests/approval.integration.test.ts`, `apps/core-brain/tests/dependency.integration.test.ts`, `apps/core-brain/tests/risk.integration.test.ts`, `apps/core-brain/tests/comment.integration.test.ts`, `apps/core-brain/tests/history.integration.test.ts`), aguardando execução em ambiente com Docker disponível — por decisão do CEO, essa execução (Parte B) será feita depois que a Fase 5 estiver completa, não a cada fatia.
...
- dashboards operacionais mínimos — último entregável da Fase 5, e todas as fases 6 em diante.

### Próximo gate recomendado

Concluir a última fatia da Fase 5 (dashboards operacionais mínimos). Só depois de a fase estar completa, confirmar em bloco a execução de todos os testes de integração e a aplicação real de todas as migrações geradas (Tasks 052 a 061 em diante, Parte B) em ambiente com Docker disponível — decisão explícita do CEO de resolver a Parte B ao final da fase, não a cada fatia. Em paralelo, decidir o modelo de multi-organização (para revisar o que já foi construído na Fase 5 sob suposição single-tenant) e a estratégia de autenticação de produção, ambas ainda pendentes.
