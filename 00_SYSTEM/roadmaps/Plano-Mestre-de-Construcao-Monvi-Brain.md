...
*Atualizado em 2026-08-19 (Task 095), após verificação direta do código, dos testes e das decisões formais — não apenas da documentação anterior desta seção.*
...
**Nota de cobertura ainda pendente** (não bloqueante, registrada para o futuro): o teste de integração de permissões (`document-permission.integration.test.ts`) não exercita o caminho de negação (403) — toda sessão criada via `createSession` resolve para o papel `admin` (join fixo em `validateSessionToken`, Fase 4), que sempre contorna a checagem granular; corrigir isso exigiria uma mudança separada no harness de sessão de teste. A Task 074 não gerou migração (`search` é rota de leitura pura).
