...
*Atualizado em 2026-08-20 (Task 097), após verificação direta do código, dos testes e das decisões formais — não apenas da documentação anterior desta seção.*
...
- Fase 3 (persistência local): schema, migração e infraestrutura Docker local implementados; validação contra um banco Postgres real **concluída** (Task 095, 2026-08-19 — via PostgreSQL portátil local, sem Docker; ver seção "Parte B" abaixo para os detalhes completos, válidos para todas as fases que dependiam dela);
- Fase 4 (identidade, autenticação dev e autorização): `dev-login` bloqueado em produção, RBAC e revogação de sessão implementados e testados (Tasks 044 e 045, 14/14 testes automatizados passando). Atribuição real de papel por pessoa (`person_role`, Task 096, 2026-08-19): até então, `validateSessionToken` resolvia toda sessão para `admin` via um join fixo (`leftJoin(role, eq(role.name, 'admin'))`) — não porque houvesse um bypass deliberado, mas porque nenhuma tabela jamais conectou pessoa a papel. `person_role` agora existe e é consultada de verdade, com fallback para `admin` quando não há atribuição explícita, preservando o comportamento de toda sessão já existente sem exigir mudança em nenhum teste. Isso destravou o único caminho de teste que dependia de Postgres real e não podia ser exercitado nem depois da Parte B (Task 095): a negação real (`403`) de acesso granular a documento, agora coberta por um teste dedicado. Atribuir papéis reais a pessoas reais do sistema — decisão de segurança de verdade, não só de testabilidade — permanece fora de escopo, para uma decisão futura separada;
...
- Hub interno (frontend), iniciativa transversal fora da numeração de Fases (ver seção 21): Task 094 entregou a primeira fatia, só leitura — login via `dev-login`, dashboard comercial, dashboard de automações e dashboard de projeto (com seleção a partir da lista). `apps/hub` (React + Vite), zero rotas novas no backend, `@fastify/cors` adicionado para permitir a chamada entre os dois servidores locais. Task 097 alinhou o visual à identidade oficial da marca Monvi (Tailwind + componentes shadcn/ui, paleta/tipografia do Manual da Marca V1.0), sem adicionar nenhuma tela ou funcionalidade. Nenhuma tela de escrita, nenhum deploy, nenhum logo real ainda (arquivo oficial não localizado).
...
### Parte B — validação real de persistência (transversal a Fases 3, 5, 6, 7, 8, 9 e 10) — RESOLVIDA (Task 095, 2026-08-19)

**Histórico**: desde a Task 052, nenhuma das migrações geradas havia sido aplicada contra um banco Postgres real, e nenhum dos testes de integração (`apps/core-brain/tests/*.integration.test.ts`) tinha sido executado com sucesso — todos falhavam com `ECONNREFUSED`, porque este ambiente de execução nunca teve Docker disponível (`infrastructure/local/docker-compose.yml` pressupõe Docker, que não foi possível instalar aqui: exige privilégio de administrador que este ambiente não tem). Ao longo de 27 tasks (Tasks 053 a 093), essa pendência foi tratada como transversal e deliberadamente adiada, com a contagem de arquivos de teste de integração rastreada task a task até chegar a 26.

**Resolução**: o CEO pediu para resolver isso na Task 094 ("Vamos fazer essa etapa e o Postgres nesse computador mesmo"). Como o ambiente não tem Docker nem permite instalar software que exija administrador, usamos os **binários portáteis do PostgreSQL 16.15** (distribuídos pela EnterpriseDB em `.zip`, sem instalador) — extraídos, inicializados (`initdb`) e iniciados (`pg_ctl start`) como processo comum do usuário, sem nenhum serviço do Windows registrado e sem senha de administrador em nenhum momento. Instalado em `C:\Users\01011610\pgsql-monvi\` (fora do repositório), escutando em `127.0.0.1:5432`, usuário `postgres`, senha `postgres_dev_password`, banco `monvi_brain_dev` — exatamente as credenciais que já eram o padrão hardcoded em `apps/core-brain/src/db/client.ts` e `drizzle.config.ts` desde o início do projeto, então nenhuma mudança de código ou de `.env` foi necessária.

**Resultado**: as 27 migrações (`0000` a `0026`) foram aplicadas com sucesso pela primeira vez — 31 tabelas criadas. Os 26 arquivos de teste de integração, com 49 testes, passaram **100%** (rodado duas vezes seguidas para confirmar estabilidade, sem flakiness). O fluxo real de login (`POST /auth/dev-login`) e os três dashboards do hub interno (Task 094) foram exercitados de ponta a ponta com um token de sessão real, incluindo a partir da origem exata do hub (`http://localhost:5173`), confirmando CORS e toda a cadeia de autenticação/autorização funcionando contra dados reais. O script de dados sintéticos já existente (`npm run db:seed`, `apps/core-brain/src/db/seed.ts` — pessoa/cliente/projeto fictícios, claramente marcados como teste) foi executado para dar ao hub algo visível além de zeros.

**Nota de infraestrutura**: este Postgres **não é um serviço do Windows** — não sobe sozinho ao reiniciar o computador; precisa ser iniciado manualmente (`pg_ctl start -D C:/Users/01011610/pgsql-monvi/data -l <log> -o "-p 5432"`) quando for necessário. Isso é aceitável para uso local de desenvolvimento; não é uma escolha adequada para produção (ver Fases 11/12, que continuam exigindo uma decisão de infraestrutura real e separada).

**Nota de cobertura, resolvida no mesmo dia (Task 096)**: o teste de integração de permissões (`document-permission.integration.test.ts`) não exercitava o caminho de negação (403) — toda sessão criada via `createSession` resolvia para o papel `admin` (join fixo em `validateSessionToken`, Fase 4), que sempre contornava a checagem granular. A Task 096 introduziu `person_role` (real, com fallback para `admin` quando não há atribuição explícita) e um teste dedicado que atribui um papel sem permissão de documento a uma pessoa de teste, confirmando o `403` de verdade pela primeira vez. A Task 074 não gerou migração (`search` é rota de leitura pura).

### Próximo gate recomendado

Com comentar e criar issue já implementados (Tasks 092/093), a lista de fatias zero-custo e autossuficientes da Fase 10 está praticamente esgotada. O próximo passo real depende de uma decisão maior do CEO: conectar um segundo provedor da lista (Google Workspace é o próximo na ordem recomendada, exige credenciais reais dele), ampliar a escrita no GitHub para ações de maior risco (labels, assignees, fechar issue, merge de PR), ou tratar uma das decisões de orçamento já registradas (embeddings da Fase 7, execução real de agentes da Fase 9, OCR). Usar as capacidades de escrita já implementadas exige que o CEO gere um `GITHUB_PAT` novo com escopo "Issues: Read and write" (o atual só tem "Contents: Read-only"). Webhook seguro de entrada (GitHub → Monvi) só se torna executável de verdade após deploy (Fases 11/12). Em paralelo, seguem pendentes, de forma transversal: a estratégia de autenticação de produção, extração real de Word legado (`.doc`), OCR de PDF digitalizado e embeddings/busca vetorial da Fase 7 (extração de texto puro concluída na Task 087, de PDF na Task 090, de Word/`.docx` na Task 091), a integração com n8n e APIs da Fase 8, e a decisão sobre execução real de agentes que destravaria os cinco entregáveis restantes da Fase 9 (avaliações, métricas de qualidade, fallback, controle de custo, proteção contra prompt injection) — a representação de `ai_agent` já está realinhada à arquitetura aprovada (Task 088), mas sua execução real segue pendente dessa mesma decisão — todas dependentes de decisões do CEO ainda não tomadas. O modelo de multi-organização não está mais nesta lista — foi formalmente decidido (single-tenant permanente, Task 086). A Parte B também não está mais nesta lista — resolvida (Task 095, validação real contra Postgres local, 49/49 testes de integração passando). O restante do modelo canônico de execução supervisionada da Fase 8 (máquina de 12 estados, retry diferenciado, separação de funções na aprovação, concorrência/locks, `cost_limit`) segue deliberadamente adiado após a Task 089, que tratou apenas do campo declarativo `timeoutSeconds`; retomar esse tema exige uma decisão explícita do CEO em conversa dedicada. O hub interno (`apps/hub`, Tasks 094/097, seção 21) segue com sua primeira fatia funcional (só leitura), agora com identidade visual alinhada à marca Monvi — telas de escrita, se vierem, são fatias futuras separadas, na mesma disciplina de escopo estreito do restante do projeto; um logo real (arquivo vetorial oficial) segue pendente de o CEO localizar/fornecer.
...
Deliberadamente fora desta fatia: qualquer ação de escrita (criar/editar/excluir), qualquer tela de CRUD completo, autenticação de produção real, deploy — o hub roda localmente, exatamente como a API roda hoje, sem nenhuma decisão de infraestrutura envolvida. Validação end-to-end com login real não foi possível neste ambiente no momento da Task 094 (mesma limitação da Parte B, então ainda não resolvida — sem Postgres local); a verificação feita foi de wiring (CORS confirmado funcionando entre os dois servidores de desenvolvimento, `dev-login` falhando exatamente com o erro de conexão já conhecido, não com um erro de integração novo) mais os testes automatizados (Vitest + React Testing Library) cobrindo o fluxo de login e a renderização dos três dashboards com dados simulados. **Atualização (Task 095, mesmo dia)**: a Parte B foi resolvida logo em seguida — o login real e os três dashboards do hub já foram validados de ponta a ponta contra um Postgres real (ver seção "Parte B" acima).

**Task 097 (2026-08-20) — identidade visual alinhada à marca Monvi**: antes de propor qualquer escopo, li o Manual da Marca Monvi V1.0 (`01_RAW/monvi/Monvi - Manual da marca.pdf`, os 15 painéis visuais originais) e os documentos derivados na Wiki (`02_WIKI/marketing/Identidade-visual.md`, `Manual-da-marca.md`, `Tom-de-voz.md`, `02_WIKI/empresa/Monvi.md`, `02_WIKI/estrategia/Posicionamento.md`, `Principio-central.md`) em vez de propor uma paleta genérica. O CEO autorizou o uso desse material (formalmente `status: review` nos metadados, nunca aprovado antes) para esta primeira versão, com alinhamento contínuo esperado. Escopo **puramente visual, sem tela nova, sem mudança de comportamento** — as mesmas 4 telas da Task 094 (login, início, três dashboards). Tailwind CSS v4 (plugin nativo do Vite) + componentes no padrão shadcn/ui, copiados para o repositório (`src/components/ui/`), não instalados como pacote; tokens de cor exatos do manual (Graphite `#424242`, Off-White `#F5F5F3`, Signal Blue `#1A4AFF` e as três cores de apoio); tipografia Inter (corpo) e IBM Plex Mono (números/dados), auto-hospedadas via `@fontsource` (sem CDN externo, mantendo o app 100% local); ícones `lucide-react` (combina com a orientação de iconografia linear/geométrica do manual). **Sem logo real** — nenhum arquivo de logotipo oficial (SVG/vetor) foi localizado no repositório; o PDF do manual não tem camada vetorial extraível, então o hub usa por enquanto só o wordmark "MONVI" em texto, substituível quando o arquivo original existir. Todos os 9 testes automatizados continuaram passando sem nenhuma alteração — confirma que foi puramente um reskin visual, não uma mudança de comportamento.

**Task 098 (2026-08-20) — modo escuro premium, fiel à marca**: o CEO enviou uma imagem de referência (dashboard cripto/Web3 em modo escuro, com glow em verde atrás dos números, cards ricos, badges) pedindo algo "visualmente mais premium". Identifiquei uma tensão real entre a referência (escura, com acento verde) e o Manual da Marca Monvi (clara, dominada por Off-White, único acento em Signal Blue) e apresentei essa tensão explicitamente em vez de só copiar a referência ou recusá-la — duas opções via pergunta estruturada. O CEO escolheu "Premium, mas fiel à marca Monvi": manter a técnica (modo escuro, glow, cards com profundidade, badges/pills), remapeando as cores para a paleta oficial, sem introduzir nenhuma cor nova. Fundo da página em Deep Graphite `#2B2B2B`, fundo dos cards em Graphite `#424242` (mais claro que o da página — cria profundidade usando só cores já oficiais), texto principal em Off-White, único acento/glow em Signal Blue (`CardGlow`, atrás dos números principais dos cards, no lugar do verde da referência), texto secundário em Medium Gray. O modo escuro passou a ser o único tema do hub. A lista simples de contagens por status/tipo (`byKey`) virou badges/pills, como na referência. Deliberadamente **não** foi replicado o gráfico de tendência histórica da referência: a API do hub só expõe contagens do momento atual, sem série temporal — fabricar dados fictícios para um gráfico teria sido inventar informação, e essa recusa foi comunicada ao CEO antes da autorização do escopo. Nenhuma tela nova, nenhuma funcionalidade nova, nenhuma mudança em `apps/core-brain`. Todos os 9 testes automatizados continuaram passando sem nenhuma alteração de asserção.

**Task 099 (2026-08-20) — sidebar de navegação, fiel à mesma imagem de referência**: o CEO revisitou a imagem de referência da Task 098 (a mesma imagem — recuperada do histórico da conversa, já que não estava mais no contexto ativo, e reanalisada) e pediu uma reformulação estrutural: "um HUB com abas laterais, opções que fazem sentido com a Monvi". A referência usa uma sidebar esquerda estreita, só com ícones (logo, Home ativo destacado, mais quatro ícones), barra superior com busca, e abas horizontais secundárias dentro da view atual. Antes de implementar, propus mapear os quatro itens que já existem de verdade no hub (Início, Comercial, Automações, Projetos, com os ícones já usados anteriormente: Home, TrendingUp, Workflow, Briefcase) para uma sidebar persistente, e perguntei se deveria ser só ícones (fiel à referência) ou ícone + texto — o CEO escolheu ícone + texto, mais explícito para um hub interno de poucos usuários. `src/components/Sidebar.tsx` (novo): sidebar fixa (~224px), item ativo destacado em `signal-blue/10`/`text-signal-blue`, "Sair" fixado no rodapé. O cabeçalho horizontal anterior foi removido de `App.tsx` — a sidebar assume a navegação principal; o layout virou `flex` (sidebar + área de conteúdo). A tela de Início deixou de ser a única forma de navegar e virou um atalho rápido complementar (mesmos três cards, texto ajustado para refletir isso). Um teste (`LoginPage.test.tsx`) verificava o texto antigo do card de Início após login real — o comportamento verificado (chegar na tela inicial autenticada) não mudou, só o texto; a asserção foi atualizada para checar o heading "Início" em vez do texto do card antigo, decisão registrada explicitamente, não uma alteração de comportamento testado. Nenhuma rota nova, nenhum `react-router` (mesmo controle de estado por `HubView` de antes), nenhuma cor nova, nenhuma chamada de API nova. Todos os 9 testes automatizados continuaram passando (1 asserção atualizada deliberadamente, 8 inalteradas).
