...
*Atualizado em 2026-08-14 (Task 084), após verificação direta do código, dos testes e das decisões formais — não apenas da documentação anterior desta seção.*
...
- Fase 10 (integrações externas): iniciada pela Task 084 com a entidade `integration` — provedor (enum fechado com as nove integrações previstas na ordem recomendada do Plano Mestre: GitHub, Google Workspace, formulários/e-mail, WhatsApp, CRM/planilhas, n8n, Nuvemshop, analytics/mídia, outras), nome, escopos mínimos pretendidos (texto livre), responsável e status (`draft`/`active`/`paused`/`revoked`, padrão `draft`) opcionais, puro cadastro — mesmo padrão que abriu as Fases 8 e 9. **Nada aqui se conecta a nenhum serviço externo de verdade**: nenhuma credencial real é armazenada, nenhuma chamada de API externa é feita, nenhum webhook real existe. Diferente de toda a base construída até aqui (que não dependia de nenhum serviço externo), os dez entregáveis restantes por integração (credenciais segregadas, escopos mínimos aplicados, webhook seguro, retry, idempotência, logs, monitoramento, revogação, documentação, teste de falha) dependem estruturalmente de decisão do CEO sobre qual serviço conectar primeiro e de credenciais reais que só ele pode fornecer.
