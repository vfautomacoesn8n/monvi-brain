...
*Atualizado em 2026-08-14 (Task 080), após verificação direta do código, dos testes e das decisões formais — não apenas da documentação anterior desta seção.*
...
- Fase 8 (plataforma de automações): iniciada pela Task 076 com a entidade `automation_workflow` — nome, descrição, responsável, tipo de gatilho pretendido (`trigger_type`: `manual`/`webhook`/`schedule`/`event`) opcionais, status (`draft`/`active`/`paused`/`archived`, padrão `draft`), puro cadastro. Estendida pela Task 077 com gatilhos e webhooks reais (segundo e terceiro entregáveis, combinados numa fatia) — `automation_trigger` referencia um `automation_workflow` e configura como ele é disparado; tipo `webhook` gera um `webhookToken` aleatório automaticamente; `scheduleCron`/`eventName` são armazenados para `schedule`/`event`, mas sem execução real. `POST /automation-triggers/:token/invoke` é a **única rota pública de todo o sistema** — sem autenticação Monvi, por decisão deliberada, já que webhooks externos reais não conseguem autenticar com Bearer token; a segurança é o próprio token no caminho, validado contra um gatilho ativo. Estendida pela Task 078 com o ciclo de vida completo de `automation_invocation` (quarto a sétimo entregáveis: fila, retries, idempotência, dead-letter, combinados por girarem em torno da mesma entidade) — `idempotencyKey` opcional em `/invoke` deduplica chamadas repetidas do mesmo evento (200 + `deduplicated: true` em vez de criar duplicata); `GET /automation-invocations/queue` lista invocações `pending` prontas para processar — a fila real, via consulta SQL, sem serviço externo; `POST /automation-invocations/:id/attempt` registra sucesso (`succeeded`, terminal) ou falha (incrementa tentativas, agenda retry com backoff exponencial, ou vira `dead_letter` ao esgotar `maxAttempts`, também terminal) — tentar uma invocação já terminal retorna 409. Continua sem nenhum worker real consumindo a fila automaticamente. Estendida pela Task 079 com aprovações e reprocessamento (oitavo e nono entregáveis, combinados por girarem em torno das mesmas duas entidades) — `automation_workflow.requiresApproval` (booleano, padrão `false`) controla se novas invocações desse workflow nascem como `pending_approval` (fora da fila, bloqueadas de `/attempt`) ou já `pending`; `POST /automation-invocations/:id/approve` decide `pending_approval` → `pending` (aprovada) ou `rejected` (terminal, com `rejectionReason` opcional), registrando `approvedByPersonId`/`approvedAt`; `POST /automation-invocations/:id/reprocess` só aceita invocações em `dead_letter`, zerando `attemptCount`/`lastError`/`nextAttemptAt` e devolvendo a `pending`. Deliberadamente não reaproveita a entidade `approval` da Fase 5 (é FK-locked a `deliverable`). Estendida pela Task 080 com logs e métricas (décimo e décimo-primeiro entregáveis) — "logs": nenhuma rota nova, apenas uma entrada adicionada ao mapa interno de `GET /history` (Task 061), já genérico, habilitando `GET /history?entityType=automation_invocation&entityId=<uuid>` sobre os eventos de `audit_event` já gravados (`received`/`attempted`/`approval_decided`/`reprocessed`); "métricas": `GET /automations/dashboard` (rota nova, mesmo padrão dos dashboards de projeto e comercial), agregando `automation_workflow` e `automation_invocation` por status via `COUNT(*) GROUP BY`. Nenhuma tabela nova, nenhuma migração gerada. Resta apenas: integração com n8n e APIs.
