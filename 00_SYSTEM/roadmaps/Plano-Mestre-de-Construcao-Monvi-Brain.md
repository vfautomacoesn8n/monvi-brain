...
*Atualizado em 2026-08-17 (Task 087), após verificação direta do código, dos testes e das decisões formais — não apenas da documentação anterior desta seção.*
...
- Fase 9 (agentes de IA), pausada por decisão do CEO — sete dos doze entregáveis implementados: catálogo (Task 081, `ai_agent` — nome, descrição, propósito, ferramentas permitidas (`allowedTools`), fontes autorizadas (`authorizedSourceIds`, sem FK, mesmo padrão de `memory_note`), responsável e status opcionais, puro cadastro, mesmo padrão que abriu a Fase 8), políticas e limites (Task 082, quinto e sexto entregáveis — `policy`, `maxActionsPerRun`/`timeoutSeconds`, controle de custo deliberadamente fora) e sinalização de aprovação humana (Task 083, sétimo entregável — `requiresHumanApproval`, ordem invertida em relação à Fase 8: o campo existe antes de qualquer execução real de agente). **Nenhum agente é de fato invocado**: não há execução de IA, chamada real a ferramentas, nem aplicação real de nenhuma política/limite/aprovação — tudo permanece metadado declarativo. O CEO decidiu pausar a fase neste ponto — avaliações, métricas de qualidade, fallback, controle de custo e proteção contra prompt injection ficam pendentes, todos dependentes de uma decisão ainda não tomada sobre se/quando construir execução real de agentes — e avançar para a Fase 10.

### Em andamento

- Fase 10 (integrações externas): iniciada pela Task 084 com a entidade `integration` — provedor (enum fechado com as nove integrações previstas na ordem recomendada do Plano Mestre: GitHub, Google Workspace, formulários/e-mail, WhatsApp, CRM/planilhas, n8n, Nuvemshop, analytics/mídia, outras), nome, escopos mínimos pretendidos (texto livre), responsável e status (`draft`/`active`/`paused`/`revoked`, padrão `draft`) opcionais, puro cadastro — mesmo padrão que abriu as Fases 8 e 9. Estendida pela Task 085 com a primeira chamada real de saída do projeto a um serviço externo — `GET /integrations/:id/github/repository?owner=<owner>&repo=<repo>`, autenticado com um Personal Access Token lido de `GITHUB_PAT` (variável de ambiente, nunca armazenada em banco), chamando `GET /repos/{owner}/{repo}` real do GitHub via `fetch` nativo (nenhuma dependência nova). Direção escolhida pelo CEO: saída (Monvi → GitHub), não entrada (webhook do GitHub → Monvi), porque o servidor roda só em `127.0.0.1` — sem URL pública, um webhook real não teria como chegar aqui antes de deploy (Fases 11/12). Escopo deliberadamente mínimo: só leitura, sem escrita, sem retry sofisticado, sem idempotência; `424` se a credencial não estiver configurada, `502` se o próprio GitHub retornar erro; toda chamada gera `audit_event`, sem o token. Diferente de toda a base construída até aqui (que não dependia de nenhum serviço externo), os oito entregáveis restantes por integração (credenciais segregadas mais robustas, escopos mínimos aplicados de fato, webhook seguro de entrada, retry, idempotência, logs/monitoramento dedicados, revogação, documentação, teste de falha) dependem de decisão do CEO sobre o que conectar em seguida — para GitHub ou para qualquer outro provedor.
...
- embeddings e busca vetorial da Fase 7 (extração de arquivos reais de texto puro concluída na Task 087; PDF/Word e embeddings/busca vetorial seguem pendentes);
...
- demais entregáveis da Fase 10 além do catálogo de `integration` e da leitura real de repositório do GitHub (credenciais segregadas mais robustas, escopos mínimos aplicados de fato, webhook seguro de entrada, retry, idempotência, logs/monitoramento dedicados, revogação, documentação, teste de falha, por integração), e todas as fases 11 em diante.

### Parte B — validação real de persistência (transversal a Fases 3, 5, 6, 7, 8, 9 e 10)

Nenhuma das migrações geradas (`0000` a `0024`, incluindo `task`, `deliverable`, `approval`, `dependency`, `risk`, `comment`, `lead`, `opportunity`, `activity`, `source`, `document`/`document_version`, `document_permission`, a política de retenção de `document`, `memory_note`, `automation_workflow`, `automation_trigger`/`automation_invocation`, o ciclo de vida de `automation_invocation`, as aprovações/reprocessamento da Task 079, `ai_agent`, suas políticas/limites da Task 082, a sinalização de aprovação humana da Task 083, `integration` e a extração real de arquivos da Task 087) foi aplicada contra um banco Postgres real; nenhum dos 26 testes de integração criados (`apps/core-brain/tests/*.integration.test.ts`) foi executado com sucesso — todos falham com `ECONNREFUSED` neste ambiente de execução, que não tem Docker disponível. A Task 079 não criou um novo arquivo de teste de integração (estendeu `automation-invocation.integration.test.ts`, já existente desde a Task 078), então a contagem permaneceu em 23; a Task 080 criou um arquivo novo (`automation-dashboard.integration.test.ts`) e estendeu `automation-invocation.integration.test.ts` de novo, elevando a contagem para 24; a Task 081 criou mais um arquivo novo (`ai-agent.integration.test.ts`), elevando a contagem para 25; as Tasks 082 e 083 estenderam esse mesmo arquivo (sem criar um novo cada vez), então a contagem permaneceu em 25; a Task 084 criou mais um arquivo novo (`integration.integration.test.ts`), elevando a contagem para 26; a Task 085 e a Task 087 estenderam arquivos já existentes (`integration.integration.test.ts` e `document.integration.test.ts`, respectivamente), sem criar um novo, então a contagem permaneceu em 26. Por decisão do CEO, essa validação é tratada como pendência única, deliberadamente adiada. Aplicar exige um ambiente com Docker: `docker compose up`, `npm run db:migrate`, `npm run test:integration`. Nota adicional: mesmo quando a Parte B for executada, o teste de integração de permissões (`document-permission.integration.test.ts`) não conseguirá exercitar o caminho de negação (403) sem uma mudança separada no harness de sessão de teste — hoje toda sessão criada via `createSession` resolve para o papel `admin` (join fixo em `validateSessionToken`, Fase 4), que sempre contorna a checagem granular. A Task 074 não gerou migração (`search` é rota de leitura pura).

### Próximo gate recomendado

Decisão do CEO sobre o próximo passo da integração real com GitHub — por exemplo, uma capacidade de escrita (comentar em PR, criar issue), o que exigiria um Personal Access Token com escopo mais amplo do que o atual (só leitura), ou decidir conectar um segundo provedor da lista (Google Workspace é o próximo na ordem recomendada). Webhook seguro de entrada (GitHub → Monvi) só se torna executável de verdade após deploy (Fases 11/12). Em paralelo, seguem pendentes, de forma transversal: a Parte B (validação real contra Postgres, cada vez mais acumulada — o CEO tentará em outra máquina com Docker disponível), a estratégia de autenticação de produção, extração real de PDF/Word e embeddings/busca vetorial da Fase 7 (extração de texto puro já concluída, Task 087), a integração com n8n e APIs da Fase 8, e a decisão sobre execução real de agentes que destravaria o restante da Fase 9 (pausada) — todas dependentes de decisões do CEO ainda não tomadas. O modelo de multi-organização não está mais nesta lista — foi formalmente decidido (single-tenant permanente, Task 086).
