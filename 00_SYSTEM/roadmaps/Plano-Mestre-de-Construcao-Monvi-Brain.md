...
*Atualizado em 2026-08-13 (Task 075), após verificação direta do código, dos testes e das decisões formais — não apenas da documentação anterior desta seção.*
...
- Fase 7 (conhecimento, documentos e memória): iniciada pela Task 069 com a entidade `source` — nome, tipo (`manual`/`upload`/`google_drive`/`website`/`api`/`other`), descrição, responsável e status (`active`/`archived`) opcionais, cadastro isolado, sem relação com nenhuma entidade das Fases 5/6. Estendida pela Task 070 com `document` e `document_version` (segundo entregável explícito, "documentos e versões") — `document_version` é imutável (sem `PATCH`/`DELETE`), com número de versão auto-incremental por documento (índice único `document_id`+`version_number`) e conteúdo textual. Estendida pela Task 071 com classificação (terceiro entregável) — `document.confidentiality` (`public`/`internal`/`confidential`/`restricted`) reaproveita exatamente o vocabulário canônico já definido em [`KNOWLEDGE-MODEL.md`](../canonical/KNOWLEDGE-MODEL.md); `document.status` realinhado ao `Status` canônico (`draft`/`review`/`approved`/`deprecated`/`archived`). Estendida pela Task 072 com permissões granulares (quarto entregável) — `document_permission` concede acesso (`read`/`write`) a uma pessoa ou a um papel sobre um documento específico; documentos `public`/`internal` continuam só com RBAC global, documentos `confidential`/`restricted` agora exigem `admin` ou concessão explícita em `GET/PATCH/DELETE /documents/:id` (403 sem ela) e são omitidos da listagem (`GET /documents`) sem concessão. Estendida pela Task 073 com política de retenção — `document.retentionPolicy` (`indefinite`/`time_limited`) e `document.retentionUntil`, validados como consistentes no mesmo pedido; puramente declarativo, sem nenhum job de descarte/arquivamento automático (isso depende da Fase 8, ainda não iniciada) — com esta task, os quatro pré-requisitos bloqueantes da regra da fase (fontes, permissões, versionamento, descarte) ficaram completos. Estendida pela Task 074 com indexação e busca textual (`GET /search?q=<termo>`, entregáveis combinados numa fatia por decisão do CEO, seguindo recomendação explícita) — busca por relevância (`ts_rank`) sobre título e conteúdo da versão mais recente, via `to_tsvector`/`plainto_tsquery` nativos do Postgres, sem tabela/coluna/dependência nova, respeitando a mesma checagem granular de `document_permission` (`hasGranularDocumentAccess` extraída para um módulo compartilhado, `src/modules/documents/access.service.ts`). Deliberadamente sem índice persistido (GIN) — calculado em tempo de consulta, aceitável no volume atual. Deliberadamente sem extração de arquivos reais (upload/parsing de PDF, Word etc.) — decisão do CEO de adiar essa frente até haver escolha de infraestrutura de armazenamento. Estendida pela Task 075 com memória operacional (`GET/POST/PATCH/DELETE /memory-notes`, penúltimo entregável da fase) — `memory_note`: anotações leves e de curto prazo, deliberadamente mais simples que `document` (sem versionamento, sem classificação, sem permissão granular); referência genérica opcional (`entityType`/`entityId`, sem FK, mesmo padrão de `audit_event`), `expiresAt` opcional e puramente informativo (sem expiração automática real, assim como a retenção de `document`). Resta apenas extração de arquivos reais como entregável explícito da fase; embeddings e busca vetorial já formalmente desbloqueados pela regra, mas ainda não implementados.
