...
describe('Integration routes — proteção de acesso (Fase 10)', () => {
  ...
  it('rejeita POST /integrations/:id/github/issues sem token com 401', async () => {
    app = await buildTestApp();

    const response = await app.inject({
      method: 'POST',
      url: '/api/v1/integrations/00000000-0000-0000-0000-000000000000/github/issues?owner=vfautomacoesn8n&repo=monvi-brain',
